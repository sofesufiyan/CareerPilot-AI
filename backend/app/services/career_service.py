...
def get_career_advice(question: str, uid: str = None) -> str:
    clear(uid)
    add("📥 User Question Received", uid)

    resume_context = ""

    if uid and should_inject_resume_context(question):
        add("🔍 Resume context intent detected.", uid)
        ...
        try:
            ...
            if history:
                ...
                resume_context = (
                    ...
                    f"Projects: {', '.join(projects[:2])}\n"
                    f"[/USER CONTEXT]"
                )

        except Exception as e:
            logger.error(
                f"Failed to fetch user context for career advice: {e}"
            )
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            add("🚀 Sending Prompt to Gemini", uid)
            ...
            add("✅ Career Response Generated", uid)
...
def review_resume(
    ...
    Runs local heuristics, attempts to retrieve Gemini structured
    recommendations, handles API service outages/timeouts gracefully
    by falling back to local scores, and persists the final structured
    review in the database.
    """

    clear(uid)
    add("📄 Resume Uploaded", uid)

    agent = ResumeAgent()
    add("🧠 Resume Agent Started", uid)

    # Local parsing metrics extraction
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            add(
                "🚀 Sending Resume Analysis to Gemini",
                uid,
            )
        ...
        logger.error(
            f"Gemini API failure or timeout: {ge}"
        )

        add(
            "⚠️ Gemini API currently busy or unavailable - "
    ...
    if ai_response is None:
        add(
            "📊 Returning Local Resume Analysis",
            uid,
        )

        ai_explanation = (
            "AI analysis is temporarily unavailable. "
            "Showing local resume analysis."
        )
        ...
        ai_explanation = parsed_data.get(
            "ai_explanation"
        )
        ...
        # Format and normalize parsed values
        roadmap = parsed_data.get("roadmap", [])

        if len(roadmap) > 6:
            roadmap = roadmap[:6]

        confidence_score = parsed_data.get(
            "confidence_score",
            90,
        )

        recommended_roles = parsed_data.get(
            "recommended_roles",
            [],
        )

        recommended_certifications = parsed_data.get(
            "recommended_certifications",
            [],
        )

        learning_resources = parsed_data.get(
            "learning_resources",
            [],
        )

        recommended_projects = parsed_data.get(
            "recommended_projects",
            [],
        )
    ...
    try:
        ...
        logger.error(
            f"Database error saving resume review: {dbe}"
        )

        add(
            f"⚠️ Database Save Failure: {dbe}",
            uid,
        )
